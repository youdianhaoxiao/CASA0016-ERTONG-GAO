...
## Motivation
Statistics show that people spend more than 90% of their time indoors. Keeping the air humid helps some people with dry skin and also requires special attention to their respiratory health. Dry environments not only cause dry skin, but also respiratory-related illnesses.Therefore, automatic humidifiers play an important role as an effective respiratory protection device for the health of some people with dry skin.
...
Here I only use a light as an indicator, I expect to use the light strip to make more fun light strip effect, for subsequent upgrades can also add more lights to achieve some different effects, the light strip for my upgrades to pave the way, for example, we subsequently add two humidifier driver module to increase the effect of humidification, the humidity is below 50% to start the two humidifiers, higher than 50% to start a humidifier, if the humidity is below If the humidity is below 40%, the three humidifiers will be turned on together, so that the back of the light strip can be used to indicate the working status of each humidifier, orange means that the corresponding humidifier is in the working state, and purple means that the humidifier is in the off state.
...
#### relay (electronics)
The reason I use a relay here is to control the humidifier on/off，the relay is used to determine whether to energise the humidifier by judging the high and low potential, when pin4 feeds back to the relay a high potential (corresponding to humidity less than 50%), the relay starts to work and energise the humidifier, when pin4 feeds back a low potential (corresponding to humidity greater than 60%), the relay breaks the circuit, and the humidifier will stop working.
